...
<table class="table table-sm">
    ...
    <tbody>
        ...
        <tr>
            <td><img src="{{ asset(env('RUTA_FOTOS') . '/' . $professor->foto) }}" width="140"></td>
            <td>{{$professor->cognoms}}, {{$professor->nom }}</td>
            <td>{{ $professor->email }}</td>
            <td>
                <a href="{{ route('professor_edit', ['id' => $professor->id]) }}" class="btn btn-dark mb-3 mx-3">Editar</a>

                <a href="{{ route('professor_delete', ['id' => $professor->id]) }}" class="btn btn-success mb-3">Eliminar</a>
