...
@section('content')
    ...
    @auth<a href="{{ route('alumne_new') }}" class="btn btn-primary">Nou alumne</a>@endauth
    </div>
    ...
    <table class="table table-sm" >
        <thead>
            <tr>
                <th>Nom</th><th>DNI</th><th>Email</th><th>Naixement</th><th>Telèfon</th><th>Grup</th><th>Mòduls</th><th>Mitjana</th><th>Accions</th>
            </tr>
        </thead>
        <tbody>
            @foreach ($alumnes as $alumne)
                <tr>
                    ...
                    <td>{{$alumne->cognoms}},{{$alumne->nom}} </td> <td>{{$alumne->dni }}</td><td>{{$alumne->email }}</td><td>{{ $alumne->data_naixement }}</td><td>{{ $alumne->telefon }}</td>
                    ...
                    <td>
                        @auth<a href="{{ route('alumne_edit', ['id' => $alumne->id]) }}"  class="btn btn-dark mb-3 mx-3">Editar</a>@endauth
                        @if(auth()->user()->rol === 'admin')<a href="{{ route('alumne_delete', ['id' => $alumne->id]) }}"  class="btn btn-success mb-3">Eliminar</a>@endif
                    </td>
                </tr>
            @endforeach
        </tbody>
